...
class AventosTypeView(APIView):
    ...
    def post(self, request):
        ...
        if listAventosHK_top_TIPON != [] and listAventosHK_top_TIPON != None:
            listAventos.update({"Поворотный подъёмник  HK Top TIP-ON": {"set": listAventosHK_top_TIPON, "tipon": True}})
        '''
        ...
        if listAventosHK_S != [] and listAventosHK_S != None:
            listAventos.update({"Малый поворотный подъёмник HK-S": {"set": listAventosHK_S, "tipon": False}})

        if listAventosHK_S_TIPON != [] and listAventosHK_S_TIPON != None:
            listAventos.update({"Малый поворотный подъёмник HK-S TIP-ON": {"set": listAventosHK_S_TIPON, "tipon": True}})

        if listAventosHK_XS != [] and listAventosHK_XS != None:
            listAventos.update({"Компактный подъёмник HK-XS": {"set": listAventosHK_XS, "tipon": False}})

        if listAventosHK_XS_TIPON != [] and listAventosHK_XS_TIPON != None:
            listAventos.update({"Компактный подъёмник HK-XS TIP-ON": {"set": listAventosHK_XS_TIPON, "tipon": True}})
